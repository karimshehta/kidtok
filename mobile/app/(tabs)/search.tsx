...
import VideoPreviewModal, { PreviewVideo } from '@/components/VideoPreviewModal'
...
export default function SearchScreen() {
  ...
  const TOP_USERS_PAGE_SIZE = 20
  const [topUsersPage, setTopUsersPage] = useState(0)
  const [allTopUsers, setAllTopUsers] = useState<UserResult[]>([])
  const [topUsersHasMore, setTopUsersHasMore] = useState(true)
  const showTopUsers = userQuery.trim().length === 0
  ...
  const [allSuggestedVideos, setAllSuggestedVideos]     = useState<SuggestedVideo[]>([])
  const [suggestedHasMore, setSuggestedHasMore]         = useState(true)

  const resetSuggestedVideos = useCallback(() => {
    setSuggestedSessionKey(`${Date.now()}-${Math.random()}`)
    setSuggestedPage(0)
    setAllSuggestedVideos([])
    setSuggestedHasMore(true)
  }, [])

  const resetTopUsers = useCallback(() => {
    setTopUsersPage(0)
    setAllTopUsers([])
    setTopUsersHasMore(true)
  }, [])

  // Reset when filters change
  useEffect(() => {
    resetSuggestedVideos()
  }, [selectedInterest?.id, selectedAge?.id, resetSuggestedVideos])

  useFocusEffect(useCallback(() => {
    if (tab === 'suggested') resetSuggestedVideos()
    if (tab === 'users' && showTopUsers) resetTopUsers()
  }, [tab, showTopUsers, resetSuggestedVideos, resetTopUsers]))

  const { isLoading: suggestedLoading, isFetching: suggestedFetching } = useQuery({
    queryKey: ['suggested-videos', selectedInterest?.id, selectedAge?.id, suggestedSessionKey, suggestedPage],
    enabled: tab === 'suggested',
    queryFn: async () => {
      const excludeIds = suggestedPage === 0 ? [] : allSuggestedVideos.map((v) => v.id)
      const { data } = await supabase.rpc('get_random_suggested_videos', {
        p_interest_id: selectedInterest?.id ?? null,
        p_age_id: selectedAge?.id ?? null,
        p_limit: SUGGESTED_PAGE_SIZE,
        p_exclude_ids: excludeIds,
      })
      const results: SuggestedVideo[] = data || []
      if (suggestedPage === 0) {
        setAllSuggestedVideos(results)
      } else {
        // Dedup by id — get_suggested_videos uses shuffle so subsequent pages
        // can return videos already loaded; without this the FlatList keys
        // collide and React throws "two children with the same key".
        setAllSuggestedVideos((prev) => {
          const seen = new Set(prev.map((v) => v.id))
          return [...prev, ...results.filter((v) => !seen.has(v.id))]
        })
      }
      setSuggestedHasMore(results.length === SUGGESTED_PAGE_SIZE)
      return results
    },
  })

  const suggestedVideos = allSuggestedVideos

  const loadMoreSuggested = useCallback(() => {
    if (suggestedHasMore && !suggestedFetching) {
      setSuggestedPage((p) => p + 1)
    }
  }, [suggestedHasMore, suggestedFetching])

  const { isLoading: topUsersLoading, isFetching: topUsersFetching } = useQuery({
    queryKey: ['top-users', userId, topUsersPage],
    enabled: tab === 'users' && showTopUsers,
    queryFn: async () => {
      const from = topUsersPage * TOP_USERS_PAGE_SIZE
      const to = from + TOP_USERS_PAGE_SIZE - 1
      ...
      const results: UserResult[] = data || []

      if (topUsersPage === 0) {
        setAllTopUsers(results)
      } else {
        setAllTopUsers((prev) => {
          const seen = new Set(prev.map((u) => u.id))
          return [...prev, ...results.filter((u) => !seen.has(u.id))]
        })
      }
      setTopUsersHasMore(results.length === TOP_USERS_PAGE_SIZE)
      return results
    },
  })

  const loadMoreTopUsers = useCallback(() => {
    if (showTopUsers && topUsersHasMore && !topUsersFetching) {
      setTopUsersPage((p) => p + 1)
    }
  }, [showTopUsers, topUsersHasMore, topUsersFetching])
  ...
  // Refresh the children list whenever the "pick child" modal opens —
  // so a child created via /children/new shows up immediately.
  useEffect(() => {
    if (step === 'child' && userId) {
      qc.invalidateQueries({ queryKey: ['children-picker', userId] })
    }
  }, [step, userId, qc])
  // Same for playlists — a freshly created playlist appears without restarting the app.
  useEffect(() => {
    if (step === 'playlist' && selectedChild?.id) {
      qc.invalidateQueries({ queryKey: ['playlists', selectedChild.id] })
    }
  }, [step, selectedChild?.id, qc])
  ...
  const searchUsers = useCallback(async (q: string) => {
    ...
    setUserResults((data || []).filter((u: UserResult) => !adminBlockedUserSet.has(u.id)))
  }, [adminBlockedUserSet])

  const visibleUserRows = useMemo(
    () => (showTopUsers ? allTopUsers : userResults).filter((u) => !adminBlockedUserSet.has(u.id)),
    [adminBlockedUserSet, allTopUsers, showTopUsers, userResults],
  )
  ...
  const onRefresh = useCallback(async () => {
    setRefreshing(true)
    resetSuggestedVideos()
    if (showTopUsers) resetTopUsers()
    await qc.refetchQueries({ queryKey: ['children-picker', userId] })
    await qc.refetchQueries({ queryKey: ['children', userId] })
    await qc.invalidateQueries({ queryKey: ['suggested-videos'] })
    await qc.invalidateQueries({ queryKey: ['top-users'] })
    setRefreshing(false)
  }, [userId, showTopUsers, resetSuggestedVideos, resetTopUsers, qc])
  ...
  const addToPlaylist = async (playlist: Playlist) => {
    ...
    try {
      ...
      Toast.show({ type: 'error', text1: ar ? 'فشل الإضافة' : 'Failed to add', text2: e.message })
      console.error(e)
    } finally {
    ...
    <KeyboardScreen variant="simple" style={{ flex: 1, backgroundColor: '#F8F9FB' }}>
      ...
      <View style={{ paddingHorizontal: spacing.lg, paddingVertical: spacing.md, backgroundColor: colors.white }}>
        <View style={{ flexDirection: 'row', gap: spacing.sm }}>
          {([
            ['suggested', ar ? 'مقترح' : 'Suggested',  colors.primary,   'sparkles-outline'],
            ...
            <Pressable
              ...
              style={({ pressed }) => ({
                ...
                backgroundColor: tab === key ? color : `${color}14`,
                alignItems: 'center', gap: 4,
                borderWidth: 1.5, borderColor: tab === key ? color : `${color}45`,
                elevation: tab === key ? 4 : 0,
                shadowColor: tab === key ? color : 'transparent',
                shadowOffset: { width: 0, height: 3 },
                shadowOpacity: 0.25, shadowRadius: 6,
                opacity: pressed ? 0.85 : 1,
                transform: [{ translateY: tab === key ? -2 : 0 }, { scale: pressed ? 0.98 : 1 }],
              })}
            >
              <Ionicons name={icon as any} size={18} color={tab === key ? '#fff' : color} />
              <Text style={{ fontWeight: '800', fontSize: 11, color: tab === key ? '#fff' : color }}>
                {label}
              </Text>
            </Pressable>
          ))}
      ...
      {tab === 'suggested' && (
        <FlatList
          ...
          // onEndReached fires once near the bottom; FlatList keeps the existing
          // scroll position when new items are appended (unlike the old
          // ScrollView+map, which jumped back to the top on every re-render).
          ...
          ListFooterComponent={
            suggestedFetching && suggestedPage > 0 ? (
          ...
          renderItem={({ item: v }) => {
            ...
            return (
              <View style={{ paddingHorizontal: spacing.lg, marginBottom: spacing.sm }}>
                <Pressable
                  onPress={() => router.push({
                    pathname: '/suggested-feed',
                    params: {
                      videoId:    v.id,
                      // Keep the user's filter context so swipe-next pulls
                      // from the same narrowed catalog, not the global one.
                      interestId: selectedInterest?.id != null ? String(selectedInterest.id) : '',
                      ageId:      selectedAge?.id      != null ? String(selectedAge.id)      : '',
                    },
                  })}
                  style={({ pressed }) => ({
                ...
                >
                  ...
                  {/* Add button — stop propagation so card tap doesn't fire */}
                  <Pressable
                    onPress={(e) => {
                      e.stopPropagation?.()
                      if (!alreadyAdded) openAddFlow({ id: v.id, title: v.title, thumbnail: v.thumbnail_url || undefined })
                    }}
                    style={({ pressed }) => ({
          ...
          <ScrollView contentContainerStyle={{ paddingHorizontal: spacing.lg, paddingBottom: 100 }}>
            {ytResults.length === 0 && !ytSearching ? (
              ...
              ytResults.map((v, _idx) => {
                const ytKey = v.id || v.youtube_id || v.video_id || String(_idx)
                const alreadyAdded = addedIds.has(ytKey)
                return (
      ...
      {tab === 'users' && (
        <View style={{ flex: 1 }}>
          ...
          <FlatList
            ...
            onEndReached={showTopUsers ? loadMoreTopUsers : undefined}
            onEndReachedThreshold={0.6}
            ListHeaderComponent={showTopUsers ? (
              <View style={{ paddingTop: spacing.sm, paddingBottom: spacing.xs }}>
                <Text style={{ color: colors.grey900, fontSize: fontSize.base, fontWeight: '900' }}>
                  {ar ? 'الأكثر متابعة' : 'Most followed'}
                </Text>
                <Text style={{ color: colors.grey400, fontSize: fontSize.xs, fontWeight: '700', marginTop: 3 }}>
                  {ar ? 'اقتراحات تقدر تتابعها وتفتح بروفايلها' : 'Suggested profiles to follow'}
                </Text>
              </View>
            ) : null}
            ListEmptyComponent={
              showTopUsers && topUsersLoading ? (
                <View style={{ alignItems: 'center', paddingTop: 60 }}>
                  <ActivityIndicator color={colors.primary} size="large" />
                </View>
              ) : (
                <View style={{ alignItems: 'center', paddingTop: 60 }}>
                  ...
                  <Text style={{ color: colors.grey400, marginTop: spacing.md }}>
                    {showTopUsers
                      ? (ar ? 'لا توجد اقتراحات حالياً' : 'No suggestions right now')
            ...
            ListFooterComponent={
              showTopUsers && topUsersFetching && topUsersPage > 0 ? (
                <View style={{ alignItems: 'center', paddingVertical: 20 }}>
                  <ActivityIndicator color={colors.primary} />
                </View>
              ) : null
              ...
              >
                ...
                <View style={{ flex: 1 }}>
                  <View style={{ flexDirection: 'row', alignItems: 'center', gap: 4 }}>
                    <Text style={{ fontWeight: '800', fontSize: fontSize.base, color: colors.grey900 }} numberOfLines={1}>
                      {u.name || (ar ? 'مستخدم' : 'User')}
                    </Text>
                    {u.is_verified && <VerifiedBadge size="sm" />}
                  </View>
                  {u.username && <Text style={{ fontSize: fontSize.sm, color: colors.primary, fontWeight: '700' }} numberOfLines={1}>@{u.username}</Text>}
            ...
            <FlatList
              ...
              ListEmptyComponent={(
                <View style={{ alignItems: 'center', padding: spacing.xl, gap: spacing.md }}>
                  ...
                  <Pressable
                    onPress={() => { closeModal(); router.push('/children/new') }}
                    style={({ pressed }) => ({
            ...
            <FlatList
              ...
              ListEmptyComponent={(
                <View style={{ alignItems: 'center', padding: spacing.xl, gap: spacing.md }}>
                  ...
                  <Pressable
                    onPress={() => { const cid = selectedChild?.id; closeModal(); if (cid) router.push(`/children/${cid}` as any) }}
                    style={({ pressed }) => ({
                      backgroundColor: pressed ? '#029ac2' : colors.primary,
                      paddingVertical: spacing.sm + 4, paddingHorizontal: spacing.lg,
                      borderRadius: radius.pill, flexDirection: 'row', alignItems: 'center', gap: 6,
