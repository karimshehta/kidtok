-- ════════════════════════════════════════════════════════════════════════════
-- Fix uploads_count + total_views for admin users who add YouTube picks
-- ════════════════════════════════════════════════════════════════════════════
--
-- Previous migration (20260526000002) counted from `creator_videos`, which
-- only contains videos uploaded by creators via Cloudflare Stream. Admins
-- who add YouTube videos to the catalog via /admin/content put rows into
-- `videos` directly with `added_by = admin_id` and `source = 'youtube'`,
-- never touching `creator_videos`.
--
-- Result: an admin like Karim with 9 YouTube picks showed 0 uploads.
--
-- Fix: count from `videos.added_by` instead. This single field captures both:
--   • Admin-added YouTube picks (source='youtube', added_by=admin_id)
--   • Creator uploads synced from creator_videos (sync trigger sets
--     added_by = new.creator_id — see migration 20260525000004)
-- So one query covers every video this user contributed to the platform.
-- ════════════════════════════════════════════════════════════════════════════
...
begin
  ...
  with filtered as (
    select
      ...
      -- Count every video this user contributed (admin picks + creator uploads).
      -- The sync trigger sets videos.added_by = creator_videos.creator_id, so
      -- this single field covers both publishing paths.
      coalesce((select count(*) from public.videos v
                where v.added_by = p.id), 0)::bigint as uploads_count,
      -- Sum views across all of this user's contributed videos.
      coalesce((select sum(v.view_count)::bigint from public.videos v
                where v.added_by = p.id), 0)::bigint as total_views,
