-- Suggested YouTube feed: keep each session random, but prefer English-titled
-- videos before Arabic-titled videos. This keeps the existing RPC signature so
-- already-built mobile clients pick up the new ordering without an app update.
...
as $$
  ...
  order by
    case
      when coalesce(v.title, '') ~ U&'[\0600-\06FF]' then 1
      else 0
    end,
    random()
