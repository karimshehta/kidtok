-- Follower milestone notifications — nudge creators to post more when
-- their followers cross a configurable step (default 20). See the
-- automation admin page for the toggle + step size.

alter table public.automation_settings
  add column if not exists follower_milestone_enabled boolean not null default true,
  add column if not exists follower_milestone_step    int     not null default 20;
