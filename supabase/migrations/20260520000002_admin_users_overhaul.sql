-- ════════════════════════════════════════════════════════════════════════════
-- Admin Users Overhaul: ban/suspend, last_active, RPC for rich admin queries
-- ════════════════════════════════════════════════════════════════════════════
...
create index if not exists idx_profiles_created_desc  on public.profiles(created_at desc);

-- ── Admin: comprehensive user list with joined stats ──────────────────────
-- Returns: user info + email + verified status + counts (videos, watch sessions, coins)
create or replace function public.admin_list_users(
  p_search          text default null,
  p_role            text default null,        -- 'parent' | 'creator' | 'admin' | null=any
  p_verified        boolean default null,     -- null=any
  p_banned          boolean default null,     -- null=any
  p_premium         boolean default null,     -- null=any
  p_has_uploads     boolean default null,     -- null=any
  p_joined_since    timestamptz default null,
  p_active_since    timestamptz default null,
  p_sort_by         text default 'created_at', -- created_at | last_active_at | name | coin_balance
...
begin
  ...
  with base as (
    select
      ...
      coalesce(p.coin_balance, 0) as coin_balance,
      ...
      (select pl.name from public.user_subscriptions us
  ...
  filtered as (
    ...
    where
      ...
      and (p_premium is null
       or (p_premium = true and b.active_plan_name is not null and b.active_plan_name not ilike '%free%')
       or (p_premium = false and (b.active_plan_name is null or b.active_plan_name ilike '%free%')))
  ),
  counted as (
...
-- ── Admin: user statistics for dashboard cards ────────────────────────────
...
begin
  ...
  select jsonb_build_object(
    ...
    'premium',   (select count(distinct us.user_id) from public.user_subscriptions us
                    join public.subscription_plans pl on pl.id = us.plan_id
                    where us.status = 'active' and pl.name not ilike '%free%'),
...
begin
  ...
  update public.profiles set coin_balance = greatest(0, coalesce(coin_balance, 0) + p_delta)
    where id = p_user_id
...
$$;
grant execute on function public.admin_adjust_coins to authenticated;
