-- Add dislike_count + comment_count to get_suggested_videos.
-- The feed's suggested tab spreads the RPC row directly into the video object,
-- so without these columns the comment/dislike counters were always 0/hidden on
-- suggested videos (while For You / Following — which read the videos table
-- directly — showed them correctly).
-- Changing the RETURNS TABLE shape requires DROP + CREATE.
drop function if exists public.get_suggested_videos(int, int, int, int);
